...
%
\just\equiv{ pela definição de num\_ops}
%
|lcbr(
		outExpAr . const X = i1
	   )(
...
%
\just\equiv{ universal-+ (segundo ramo)}
%
|lcbr(
		outExpAr . const X = i1
	   )(
		lcbr(outExpAr . N = i2 . i1)( outExpAr . ops = i2 . i2)
	   )|
%
\just\equiv{ pela definição de ops}
%
|lcbr(
		outExpAr . const X = i1
	   )(
		lcbr(outExpAr . N = i2 . i1)( outExpAr . [bin, uncurry un] = i2 . i2)
	   )|
%
\just\equiv{universal-+ (terceiro ramo)}
%
|lcbr(
		outExpAr . const X = i1
	   )(
		lcbr(outExpAr . N = i2 . i1)
    (lcbr(outExpAr . bin = i2 . i2 . i1)(outExpAr . uncurry un = i2 . i2 . i2))
	   )|
%
\just\equiv{universal-+ (Igualdade extensional, Def-comp)}
%
|lcbr(
		outExpAr X = i1 ()
	   )(
		lcbr(outExpAr (N a) = i2 (i1 a) )
        (lcbr(outExpAr (Bin x y z) = i2(i2(i1 (x,(y,z)))))
             (outExpAr (Un x y) = i2(i2(i2 (x,y)))))
	   )|
\end{eqnarray*}
...
Observando a expressão do catamorfismo que nos foi fornecida no enunciado, verificamos que irá ser aplicado ao recExpAr o functor em que o tipo de 
entrada é a estrutura de dados, e o resultado do functor é a aplicação da recursividade quando existe uma expressão e id nos restantes casos.
\begin{code}
recExpAr g = baseExpAr id id id g g id g
\end{code}

De forma a que se tornasse mais fácil definir a função g\_eval\_exp decidimos desenvolver o diagrama de catamorfismo da função eval\_exp, 
uma vez que g\_eval\_exp corresponde ao seu gene:
\begin{eqnarray*}
\xymatrix@@C=2cm{
    |ExpAr a|
           \ar[d]_-{|eval_exp|}
...
\begin{code}
g_eval_exp x = either g1 (either g2 (either g3 g4)) where
        g1 = const x
        g2 a = a
        g3 (Sum,(a,b)) = a + b
        g3 (Product,(a,b)) = a * b
        g4 (Negate,a) = (-1) * a
        g4 (E,a) = Prelude.exp a

\end{code}
Sabendo que a função clean é o gene do anamorfismo, decidimos desenvolver o diagrama do mesmo de forma 
a ser mais fácil definir a função em questão:
\begin{eqnarray*}
\xymatrix@@C=2cm{
    |ExpAr A|
&
...
&
     |1 + A + BinOp >< A >< A + UnOp >< A|
            \ar[u]_{|recExpAr (anaExpAr clean)|}
}
\end{eqnarray*}

\begin{code}
clean (Bin Product a b) = if a == N 0 || b == N 0 then i2(i1(0)) else outExpAr (Bin Product a b) 
clean x = outExpAr x
\end{code}


O gopt é igual à alínea 2 porque continuamos a quere calcular o valor da expressão da mesma forma que calculamos para p g\_eval\_exp:
\begin{code}
gopt x = g_eval_exp x        
\end{code}
...
sd_gen :: Floating a =>
    ...
    -> (ExpAr a, ExpAr a)
sd_gen = either (split (const X)  (const (N 1))) (either (split N (const (N 0))) (either g3 g4)) where
        g3 = split e1 e2  where
            e1 (op,((a1,a2),(b1,b2))) =  Bin op a1 b1 
            e2 (Sum,((a1,a2),(b1,b2))) =  Bin Sum a2 b2
            e2 (Product,((a1,a2),(b1,b2))) = Bin Sum (Bin Product a1 b2) (Bin Product a2 b1)
        g4 = split e3 e4 where
...
\begin{code}
ad_gen x = either g1 (either g2 (either g3 g4)) where
        g1 () = (x,1)
        g2 n = (n,0)
        g3 (Sum,((eo1,ed1),(eo2,ed2))) = (eo1 + eo2, ed1 + ed2)
...
\end{code}
seja a função pretendida.\par
Analisando a fórmula que dá o |n|-ésimo \catalan{número de Catalan},
\begin{eqnarray}
	C_n = \frac{(2n)!}{(n+1)! (n!) }
\end{eqnarray}
...
\begin{eqnarray}
	C_n = \frac{C_1}{C_2}
\end{eqnarray}
Assim sendo, começamos por obter $c_1 0$ e $c_1 n+1$ com $c_1 = (2n)!$:

\begin{spec}
  c1 0 = (2*0)! = 0! = 1
  c1 n+1 = (2(n+1))! = (2n + 2)! = (2n + 2) * (2n + 1) * (2n)! = (4 n * n + 6n + 2) * c1 n = w * c1 n
\end{spec}
sendo de seguida necessário o mesmo processo para $w = (4n^2 + 6n + 2)$:
\begin{spec}
  w 0 = (2 * 0 + 2) * (2 * 0 + 1) = 2 * 1 = 2
  w n+1 = (2n + 2 + 2) * (2n + 2 + 1) = (2n + 4) + (2n + 3) = 4 n * n + 14n + 12 = w n + x
\end{spec}  
...
\begin{spec}
  ...
  x n+1 = 8n + 10 + 8 =  x n + 8
\end{spec}  
Repetindo o processo para obter $c_2 0$ e $c_2 n+1$ com $c_2 = (n+1)! (n!)$:
\begin{spec}
c2 0 = 1!0! = 1
...
\begin{spec}
y 0 = 2
y n+1 = (n+1)^(n+1) + (3n+3) + 2 = n*n + 5n + 6 = h2 n + z
\end{spec}  
sendo $z = 2n + 4$:
\begin{spec}
z 0 = 4
...
\begin{code}
loop (c1,w,x,c2, y, z) = (c1 * w, w + x, x+8, c2*y, y+z,z+2)
inic = (1,2,10,1,2,4)
prj (c1,w,x,c2,y,z) = div c1 c2
\end{code}
...
\xymatrix@@C=2cm{
      |[NPoint]|
           \ar[d]_-{|anaList coalg|}
           \ar[r]^-{|coalg|}
&
    |Either 1 a >< [NPoint]|
           \ar[d]^{|id + id >< (anaList coalg)|}
\\
    |[a]|
           \ar[d]_-{|cataList alg|}
           \ar[r]_-{|outList|}
&
    |Either 1 a >< [a]|
           \ar[d]^{|id + id >< (cataList alg)|}
\\
     |OverTime NPoint|
&
     |Either 1 a >< OverTime NPoint|
           \ar[l]^-{|alg|}
}
\end{eqnarray*}
...
Redefinição das funções de listas, de forma a permitir listas com um só elemento 
(caso de paragem deixa de ser a lista vazia e passa a ser lista só com um elemento):  

\begin{code}

inList2 = either singl cons

outList2 [a] = i1 a
outList2 (a:x) = i2(a,x)

cataList2 g = g . recList2 (cataList2 g) . outList2   
...
&
     |Nat0 + Nat0 >< Nat0 >< Nat0|
           \ar[l]^-{|split (either (const 1) (succ . p2 . p2)) (either id alpha)|}
}
\end{eqnarray*}
...
\begin{eqnarray*}
\start
	|split avg length = cata (split (either (const 1) (succ . p2 . p2)) (either id alpha))|
%
\just\equiv{ Lei da troca}
%
  |split avg length = cata (either (split (const 1) (id)) (split (succ. p2 . p2 ) (alpha)))|
%
\just\equiv{ Através da observação da expressão podemos concluir que:}
%
  |avg = p1 . cata (either (split (const 1) (id)) (split (succ. p2 . p2 ) (alpha)))|
\end{eqnarray*}


\begin{code}
avg_aux = cataList2 (either e1 e2) where 
      e1 = split id (const 1)
      e2 = split a (succ . p2 . p2)
...
&
    |A + LTree A >< LTree A|
           \ar[d]^{|id + split avg length >< split avg length|}
\\
     |Nat0|
&
     |A + Nat0 >< Nat0 >< Nat0 >< Nat0|
           \ar[l]^-{|split (either (const 1) (p2 >< p2)) (either id alpha)|}
}
\end{eqnarray*}
...
\begin{eqnarray*}
\start
	|split avg length = cata (split (either (const 1) (p2 >< p2)) (either id alpha))|
%
\just\equiv{ Lei da troca}
%
  |split avg length = cata (either (split (const 1) (id)) (split (p2 >< p2 ) (alpha)))|
%
\just\equiv{ Através da observação da expressão podemos concluir que:}
%
  |avg = p1 . cata (either (split (const 1) (id)) (split (p2 >< p2 ) (alpha)))|
\end{eqnarray*}
